...
void sr_arpcache_handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req)
{
    ...
    */
    ...
    if (difftime(curtime, req->sent) > 1.0)
    {
        if (req->times_sent >= 5)
        {
            /* send ICMP host unreachable (type 3, code 1) to all waiting packets */
            for (pck = req->packets; pck != NULL; pck = pck->next)
            {
                struct sr_ethernet_hdr *e_hdr0 =
                    (struct sr_ethernet_hdr *)pck->buf;
                i_hdr0 = (struct sr_ip_hdr *)(pck->buf +
                ...
                ict3_hdr = (struct sr_icmp_t3_hdr *)(buf +
                            sizeof(struct sr_ethernet_hdr) +
                            sizeof(struct sr_ip_hdr));
                ...
                /* choose outgoing interface back to original source */
